...
export class QuarterbackService {
    ...
    QUARTERBACKS.forEach((qb, i) => {
      ...
      qb.rows.forEach((q) => {
        ...
        gameData.completionPercentage =
          gameData.completions / gameData.attempts;
        gameData.yardsPerAttempt =
          Math.ceil((gameData.passingYards / gameData.attempts) * 10) / 10;
  ...
  private calculateSeasonStats(gameStats: IGameData[]): ISeasonStats {
    const seasonStats: ISeasonStats = {
      ...
      completions: gameStats
        .filter((gs) => gs.completions)
        .reduce((a, b) => a + b.completions, 0),
      sacks: gameStats
        .filter((gs) => gs.sacks)
        .reduce((a, b) => a + b.sacks, 0),
      interceptions: gameStats
        .filter((gs) => gs.interceptions)
        .reduce((a, b) => a + b.interceptions, 0),
      passingYards: gameStats
        .filter((gs) => gs.passingYards)
        .reduce((a, b) => a + b.passingYards, 0),
      passingTds: gameStats
    ...
    seasonStats.completionPercentage =
      seasonStats.completions / seasonStats.attempts;
    seasonStats.yardsPerAttempt =
      Math.ceil((seasonStats.passingYards / seasonStats.attempts) * 10) / 10;
